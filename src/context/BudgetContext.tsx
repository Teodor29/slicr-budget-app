...
import { loadData, saveData, getCurrentMonthKey, createInitialData } from '../lib/storage';
...
export function BudgetProvider({ children }: { children: ReactNode }) {
  const [data, setData] = useState<AppData>(() => loadData() ?? createInitialData());
  const [viewedMonth, setViewedMonth] = useState(() => (loadData() ?? createInitialData()).currentMonth);
  ...
  const addTransaction = useCallback((t: Omit<Transaction, 'id'>) => {
    ...
    update({
      ...
      months: {
        ...
        [viewedMonth]: {
          ...month,
          transactions: [...month.transactions, { ...t, id: crypto.randomUUID() }],
        },
      },
    });
  ...
  const updateTransaction = useCallback((t: Transaction) => {
    ...
    update({
      ...
      months: {
        ...
        [viewedMonth]: {
          ...month,
          transactions: month.transactions.map(tx => tx.id === t.id ? t : tx),
        },
      },
    });
  }, [data, viewedMonth, update]);
  ...
  const addCategory = useCallback((cat: Omit<Category, 'id'>) => {
    ...
    update({
      ...
      months: curMonth ? {
        ...data.months,
        [data.currentMonth]: { ...curMonth, categories: [...curMonth.categories, newCat] },
      } : data.months,
    });
  }, [data, update]);
  ...
  return (
    <BudgetContext.Provider value={{
      data, viewedMonth, setViewedMonth,
      pendingNewMonth, confirmNewMonth, dismissNewMonth,
